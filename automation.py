...
RESULT_BOXES = [
    (*_r1tl, *_r1br), (*_r2tl, *_r2br),
    (*_r3tl, *_r3br), (*_r4tl, *_r4br),
]
...
QUAD_CLICKS = {1: QUAD_CLICK_TL, 2: QUAD_CLICK_TR, 3: QUAD_CLICK_BL, 4: QUAD_CLICK_BR}
...
BACKGROUND_COLOR_RGB = (0x0c, 0x23, 0x40)
...
OVR_MAX_CLICK_POSITIONS = {79: BTN_FILTER_OVR_MAX_VALUE, 84: BTN_FILTER_OVR_MAX_84}
...
def click(pos, delay=1.0):
    adb_screen.tap(pos[0], pos[1], delay)

def click_and_wait(pos, delay=2.5):
...
def _is_stubs_logo_color(r, g, b):
...
def get_search_term(full_name):
    ...
    first_clean, last_clean = strip_accents(first), strip_accents(last)
    if last == last_clean:
        return last
...
def find_card_in_results(target_name):
    ...
    for i, box in enumerate(RESULT_BOXES):
        ...
        matched = target_clean in ocr_clean
...
# ─── Order popup detection ─────────────────────────────────────────────────

def _wait_for_order_popup():
    start = time.time()
...
def _cancel_visible_orders(label: str) -> int:
    ...
    Uses popup detection to handle orders that complete mid-cancel.
    """
    cancelled = 0
    max_failures = 3
    ...
    while True:
        ...
        adb_screen.tap(BTN_CONFIRM_CANCEL[0], BTN_CONFIRM_CANCEL[1], delay=0.1)

        popup = _wait_for_order_popup()

        if popup == "green":
            print(f"    ✓ Cancel confirmed.")
            cancelled += 1
            consecutive_failures = 0
            time.sleep(1.0)
        elif popup == "red":
            print(f"    Order already completed — skipping.")
            consecutive_failures += 1
            time.sleep(1.0)
        else:
            print(f"    Cancel popup timeout — assuming order completed.")
            consecutive_failures += 1
            time.sleep(1.0)

        if consecutive_failures >= max_failures:
            print(f"    {max_failures} consecutive cancel failures — moving on.")
            break
...
def buy_one_card(name, uuid, rarity="silver", is_duplicate_name=False, min_profit=0):
...
def run_buy_orders(cards, skip_clear=False, skip_names=None, min_profit=60,
                   rarity="silver", skip_navigate=False):
    ...
    for i, card in enumerate(cards):
        name, uuid = card["name"], card["uuid"]
        est_price, profit = card["sell_now"] + 1, card["spread"]
        ...
        elif result["reason"] in ("not_found", "no_price", "low_profit"):
            skipped += 1
        else:
            errors += 1
...
def has_card_in_quad(quad_num):
    adb_screen.invalidate_cache()
    img = adb_screen.screenshot()
    return _has_card_in_quad_from_image(quad_num, img)

def _has_card_in_quad_from_image(quad_num, img):
...
def _store_unsellable_fingerprint(fp):
    if fp:
        _get_unsellable_fps().append(fp)

def _learn_fingerprint_from_grid(quad_num):
    adb_screen.invalidate_cache()
...
def _has_dupe_icon(quad_num, img):
...
def scroll_inventory_down():
    ...
    adb_screen.swipe(SCROLL_DOWN_START[0], SCROLL_DOWN_START[1],
                     SCROLL_DOWN_END[0], SCROLL_DOWN_END[1], duration_ms=300, delay=0.5)

def scroll_inventory_up():
    adb_screen.swipe(SCROLL_DOWN_END[0], SCROLL_DOWN_END[1],
                     SCROLL_DOWN_START[0], SCROLL_DOWN_START[1], duration_ms=300, delay=0.3)


# ─── Sellability checks ──────────────────────────────────────────────────

def _check_menu_button_exists():
    color = get_pixel_color(MENU_BTN_CHECK[0], MENU_BTN_CHECK[1])
    try:
        r, g, b = int(color[0:2], 16), int(color[2:4], 16), int(color[4:6], 16)
        tr, tg, tb = int(MENU_BTN_COLOR[0:2], 16), int(MENU_BTN_COLOR[2:4], 16), int(MENU_BTN_COLOR[4:6], 16)
        match = abs(r-tr) <= MENU_BTN_TOLERANCE and abs(g-tg) <= MENU_BTN_TOLERANCE and abs(b-tb) <= MENU_BTN_TOLERANCE
...
def swipe_refresh(scrolls_to_reverse=0):
    ...
    adb_screen.swipe(SWIPE_START[0], SWIPE_START[1], SWIPE_END[0], SWIPE_END[1],
                     duration_ms=400, delay=3.0)

def click_quad(quad_num):
    click_and_wait(QUAD_CLICKS[quad_num], 2.5)
...
def _get_uuids_from_map(uuid_map, card_name, rarity):
    import re
...
def _pick_uuid_by_price(uuids, screen_price):
    best_uuid, best_diff = None, float('inf')
    for uid in uuids:
        try:
            listing = fetch_single_listing(uid)
            buy_now_raw = listing.get("best_sell_price")
            if buy_now_raw is None or buy_now_raw == "-": continue
            api_price = int(buy_now_raw)
            diff = abs(api_price - screen_price)
            print(f"      UUID {uid[:12]}... → buy_now={api_price:,} (diff={diff:,})")
            if diff < best_diff:
                best_diff, best_uuid = diff, uid
        except Exception as e:
...
def _apply_ovr_filter(min_ovr=80, max_ovr=None):
    label = f"OVR {min_ovr}-{max_ovr}" if max_ovr else f"OVR {min_ovr}+"
    ...
    for i in range(num_swipes):
        adb_screen.swipe(BTN_FILTER_OVR_NUMBER[0], BTN_FILTER_OVR_NUMBER[1],
    ...
    if max_ovr is not None:
        ...
        swipe_y, swipe_x = BTN_FILTER_OVR_MAX_NUMBER[1], BTN_FILTER_OVR_MAX_NUMBER[0]
        adb_screen.swipe(swipe_x, swipe_y, swipe_x, swipe_y - 400, duration_ms=500, delay=1.0)
...
def _navigate_to_inventory_filtered(min_ovr=80, max_ovr=None):
    print("  Resetting profile page...")
    click_and_wait(BTN_PROFILE, 1.5)
    click_and_wait(BTN_PROFILE, 3.0)
...
def _reset_inventory_filtered(min_ovr=80, max_ovr=None):
...
def _mark_unsellable(unsellable, name, price, no_market=False):
...
def _is_known_unsellable(unsellable, name, price):
    if name not in unsellable: return False
    buckets = unsellable[name]
    if price is None: return _NO_MARKET_BUCKET in buckets
    return _price_bucket(price) in buckets


# ─── Main sell loop ────────────────────────────────────────────────────────

def run_sell_orders(skip_clear=False, include_silver=False,
                    silver_only=False, gold_silver=False, max_scrolls=None,
                    max_passes=None):
    """Sell flow — sells cards from OVR-filtered inventory.
    
    max_passes: if set, stop after this many passes even if sellable cards remain.
    """
    uuid_map = load_uuid_map()
    if not uuid_map:
        return
    ...
    total_sold, total_skipped, total_errors = 0, 0, 0
    sold_names = []
    unsellable = {}
    pass_num = 0
    ...
    while True:
        ...
        print(f"\n{'=' * 55}")
        print(f"  SELL PASS #{pass_num} (known unsellable: {unsellable_count}, fingerprints: {len(fps)})")
        print(f"{'=' * 55}")
        ...
        while not hit_end:
            ...
            print(f"\n  --- Checking quads {start_quad}-4 (scroll {scrolls_done}, sold this pass: {sold_this_pass}) ---")
            ...
            if not hit_end and not quads_to_process:
                ...
                if scrolls_done > 0:
                    ...
                    if prev_bottom_fps and len(bottom_fps_list) == len(prev_bottom_fps):
                        all_same = all(_fingerprints_match(a, b) for a, b in zip(bottom_fps_list, prev_bottom_fps))
                        if all_same:
            ...
            for q in quads_to_process:
                ...
                for try_rarity in rarity_search_order:
                    ...
                    if len(uuids) == 1:
                        if screen_price is not None:
                            try:
                                ...
                                if prices_match(screen_price, api_price):
                                    print(f"    {try_rarity.title()} UUID price verified: screen={screen_price:,} ≈ API={api_price:,}")
                                    sell_rarity, uuid = try_rarity, uuids[0]
                                    break
                                else:
                                    print(f"    {try_rarity.title()} price mismatch: screen={screen_price:,} vs API={api_price:,} — trying next rarity")
                                    continue
                            except Exception:
                                pass
                        sell_rarity, uuid = try_rarity, uuids[0]
                        ...
                        if screen_price is not None:
                            ...
                            if matched:
                                sell_rarity, uuid = try_rarity, matched
                                break
                            else:
                ...
                while True:
                    ...
                    if popup == "green":
                        ...
                        adb_screen.invalidate_cache()
                        fresh_img = adb_screen.screenshot()
                        if _has_card_in_quad_from_image(q, fresh_img) and _has_dupe_icon(q, fresh_img):
        ...
        unsellable_count = sum(len(v) for v in unsellable.values())
        print(f"\n  Pass #{pass_num} complete: sold {sold_this_pass}, unsellable {unsellable_count}")
        ...
        if max_passes is not None and pass_num >= max_passes:
            print(f"  Reached max passes ({max_passes}) — breaking to cancel orders.")
            break
